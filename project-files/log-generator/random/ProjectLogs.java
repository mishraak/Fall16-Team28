...
class ProjectLogs {
	private static final String FILENAME= "/var/log/app/iis.log";
	...
	public static int[] year={2012,2013,2014,2015};
		...
		try {
			//	fw= new FileWriter(ProjectLogs.FILENAME,true);
				...
				for(int k=0;k<m;k++) {
					...
					bw.write(ProjectLogs.getRandom(year)+"-"+fmt.format(ProjectLogs.randomInteger(1, 12))+"-"+fmt.format(ProjectLogs.randomInteger(1, 28))+"T"+fmt.format(ProjectLogs.randomInteger(0, 23))+":"+fmt.format(ProjectLogs.randomInteger(0,59 ))+":"+fmt.format(ProjectLogs.randomInteger(0,59))+"Z"+" 172.224."+ProjectLogs.randomInteger(0,254)+"."+ProjectLogs.randomInteger(0,254)+" "+ProjectLogs.getrandstring(users)+" "+"206.73.118."+ProjectLogs.randomInteger(0,254)+" "+port+" "+ProjectLogs.getrandstring(reqs)+" "+"-"+" "+ProjectLogs.getRandom(x)+" "+ProjectLogs.randomInteger(0,4000)+" "+ProjectLogs.randomInteger(0,4000)+" "+ProjectLogs.randomInteger(0,600));
					bw.newLine();
					try{
						if (bw != null)
						bw.close();
